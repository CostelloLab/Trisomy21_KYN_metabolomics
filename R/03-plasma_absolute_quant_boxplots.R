# -----------------------------------------------------------------------------
# Script 03: Human Plasma KYN, TRP, and QUIN (absolute quant)
# Author: Rani Powers
#
...
dat = read.csv('Data/Human_plasma_metabolomics_absolute.csv',
...
write.table(pvals_table2, 'Results/Supplementary_Files/Supplementary_Data_6.csv',
              sep = ',', row.names = F)
...
compounds = c('KYN', 'Quinolinic_acid', 'KYNTRP')
for (compound in compounds){
  ...
  names(df)[2] = compound
  pdf(paste0('Results/Figure_1/Fig_1D_boxplot_', compound, '.pdf'), 
      height = 6, width = 6)
  par(mar = c(5,5,5,2))
